...
**seataudit** plugs into your stack through [`cognis-connect`](https://github.com/cognis-digital/cognis-connect),
...
Once `seataudit` emits JSON findings, pipe them straight to a destination — `--dry-run`
previews the exact request without sending:

```bash
seataudit ... --format json | cognis-connect emit --to sigma   # Sigma rules
seataudit ... --format json | cognis-connect emit --to splunk --url $URL --token $TOK   # Splunk HEC
seataudit ... --format json | cognis-connect emit --to elastic --url $URL --token $TOK   # Elastic _bulk
seataudit ... --format json | cognis-connect emit --to webhook --url $URL --token $TOK   # generic webhook
```
...
works whatever `seataudit` outputs:

```python
from cognis_connect import normalize, sigma
findings = [normalize(rec, source="seataudit") for rec in records]   # records = your JSON output
